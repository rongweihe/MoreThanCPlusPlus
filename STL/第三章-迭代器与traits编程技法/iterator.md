...
![](https://cdn.jsdelivr.net/gh/rongweihe/ImageHost01/images/stl-iterator-mind.png)

在 STL 编程中，容器和算法是独立设计的，容器里面存的是数据，而算法则是提供了对数据的操作，在算法操作数据的过程中，要用到迭代器，迭代器可以看做是容器和算法中间的桥梁。

![](https://cdn.jsdelivr.net/gh/rongweihe/ImageHost01/images/stl-iter-con.png)
...
![gp-and-oop.png](https://cdn.jsdelivr.net/gh/rongweihe/ImageHost01/images/gp-and-oop.png)
...
- (2) 对于 `list` ，取下一个元素不是通过自增而是通过 `next`  指针来取，使用智能指针可以对自增进行重载，从而**提供统一接口**。
...
![](https://cdn.jsdelivr.net/gh/rongweihe/ImageHost01/images/stl-iterator-temp.png)
...
```
![](https://cdn.jsdelivr.net/gh/rongweihe/ImageHost01/images/stl-iterator-01.png)

上面的解决方案看着可行，但其实呢，实际上还是有问题，这里有一个隐晦的陷阱：**实际上并不是所有的迭代器都是 `class type` ，原生指针也是一种迭代器，由于原生指针不是 `class type` ，所以没法为它定义内嵌型别**。

![](https://cdn.jsdelivr.net/gh/rongweihe/ImageHost01/images/stl-iterator-02.png)
...
## 4、Partial specialization（模板偏特化）
...
### 4.1、原生指针怎么办？——特性 “萃取” traits

还记得前面说过的**参数推导机制+内嵌型别机制获取型别**有什么问题吗？问题就在于原生指针虽然是迭代器但不是`class` ，无法定义内嵌型别，而偏特化似乎可以解决这个问题。

有了上面的认识，我们再看看 `STL`  是如何应用的。`STL`  定义了下面的类模板，它专门用来“萃取”迭代器的特性，而`value type`  正是迭代器的特性之一：
...
![stl-iterator-09.png](https://cdn.jsdelivr.net/gh/rongweihe/ImageHost01/images/stl-iterator-09.png)
...
![](https://cdn.jsdelivr.net/gh/rongweihe/ImageHost01/images/stl-traits-10.png)
...
![](https://cdn.jsdelivr.net/gh/rongweihe/ImageHost01/images/stl-traits-all.png)
...
![](https://cdn.jsdelivr.net/gh/rongweihe/ImageHost01/images/traits-program-type2.png)
...
![](https://cdn.jsdelivr.net/gh/rongweihe/ImageHost01/images/iter-type-all.png)
